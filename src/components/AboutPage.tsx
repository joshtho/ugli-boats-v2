import React from 'react'

function AboutPage() {
  return (
    
    <div className="bg-muted/50 min-h-[100vh] flex-1 rounded-xl md:min-h-min">
      <h1 className="text-2xl font-bold p-4">How this all started</h1>
      <h1 className="text-xl text-left p-4">A Couple of Ugli guys</h1>
      <p className="p-4">
        My love for the ponton bridge half-section boat was developed when I let a neighbor, who was moving, store his ponton bridge boat at my house. He was very proud of it and wouldn't sell it for less than $3000.
        <br />
        <br />
        At that time I thought he had it way over priced for a big "Jon-Boat". During the three years that he left it here, I kept offering more money, but I could never bring myself to spend that much on a hull. I had named it FISHGUTS.
        <br />
        <img src='/IMAGES/AUpage fishguts pic.jpg' />
        <br />
        When my ex-neighbor finally came and got his boat - I almost immediately regretted not grabbing it when I had the chance.
        <br />
        <br />

        I spent months researching online to find a used "store-bought" equivalent that was at least 18.5' long and 6' wide at the floor. It also had to have sides high enough to take out in big water, and low enough and flat bottomed enough to easily fish out of.
        <br />
        <br />

        Let me tell you... I could not find anything at a reasonable price that had all the great features of these old ponton bridge boats. I eventually changed gears and began a relentless search for my own ponton bridge boat. My dreams of finding a surplus auction where I could get one for $300 faded very quickly.
        <br />
        <br />

        I tracked down many false leads, scouring all the government auction sites and calling military bases nationwide. It wasn't too long before I realized that the days of buying a stack of these at a military surplus auction are long gone. I didn't have any luck finding one, but my luck was starting to change when I struck up a friendship with a guy that is a good boat mechanic, and quite a craigslist boat section expert.
        <br />
        <br />

        For a hobby, we started dabbling in buying, fixing-up and selling old boats. I told him about Ol' Fishguts - (The One That Got Away), and he started to keep an eye out for another one. After about a year of searching, he called late one evening and said he found my dream, less than 50 miles away. The ad said nothing about "army boat" "ponton bridge boat" "big aluminum boat" or anything remotely Google-able. The ad simply read; "OLD FISHING BOAT". Luckily there was a picture and he recognized the boat.
        <br />
        <br />

        When I saw it I decided to call her "UgliBoat". It had an old 100 Johnson on it, center console, and a rotted floor. The transom had been cut out and heavy steel reinforcements were attached. The guy wanted $2500, and even though I usually try to get a bargain, I didn't even haggle over the price.

        <br />
        <br />
        <hr />
        <br />
        This is the picture of UgliBoat #1 the day I bought it. Many thanks to my friends wife, Michelle, who saved these early pictures.
        <br />
        <br />
      <img src='/IMAGES/AU Ugli1 purchase day.jpg' />
        <br />

        Once we started working on UgliBoat #1 we really caught the bug for these boats. I sold Ugli #1 because of financial reasons, but am currently working on UgliBoat #2 which also took over a year to locate.

        Since I had to drive all the way to Oklahoma for Ugli#2, and have been accumulating all the expensive new toys to put on it - Ugli2 is for keeps !

        I will have so much time and money in Ugli2, that I may just decide to be buried in it.
      </p>
    </div>
  )
}

export default AboutPage